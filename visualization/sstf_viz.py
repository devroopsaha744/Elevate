...
class SSTFElevator(Scene):
    def construct(self):
        ...
        algorithm_text = Text("SSTF Algorithm", font_size=24).to_edge(DL)
        ...
        # Animate the elevator according to the SSTF algorithm
        self.wait(1)
        ...
        while remaining_requests:
            ...
            closest_request = min(remaining_requests, key=lambda x: abs(x - current_floor))
            
            # Move elevator to the closest requested floor
            distance = abs(closest_request - current_floor)
            ...
            self.play(move_elevator_to(closest_request))
            self.play(request_numbers[requests.index(closest_request)].animate.set_color(PINK))
            self.wait(0.5)
            
            remaining_requests.remove(closest_request)
            current_floor = closest_request
